...
class Antidote(pygame.sprite.Sprite):
	img = pygame.image.load('antidote.png')

	def __init__(self, game):
		self.game = game
		self.groups = self.game.antidotes, self.game.allsprites
		pygame.sprite.Sprite.__init__(self, self.groups)

		self.image = Antidote.img
...
class Medusa(pygame.sprite.Sprite):
	img = pygame.image.load('medusa.png')
	...
	def __init__(self, game, room):
		...
		self.image = Medusa.img
		self.rect = self.image.get_rect()
		self.rect.center = (int(self.game.screenwidth / 4), int(self.game.screenheight / 2))

		self.curr_room = room
		self.last_move = time.time()
		self.hopping_interval = 30

	def move_room(self):
		new_room = random.choice(self.curr_room.doors).destination
		self.curr_room.medusa = False
		self.curr_room = new_room
		self.curr_room.medusa = True
		self.last_move = time.time()

	def update(self):
		if time.time() - self.last_move >= self.hopping_interval:
			self.move_room()
